...
src_path = path.join(path.dirname(__file__),'..','SparkFunKiCadBOMGenerator')
...
md['versions'][0].update({
    'version': verstr,
    'download_url': 'https://github.com/sparkfun/SparkFun_KiCad_BOM_Generator/releases/download/v{0}/SparkFunKiCadBOMGenerator-{0}-pcm.zip'.format(verstr)
})
...
# Zip all files
zip_file = 'SparkFunKiCadBOMGenerator-{0}-pcm.zip'.format(md['versions'][0]['version'])
...
shutil.move('plugin', 'SparkFunKiCadBOMGenerator-{0}-pcm'.format(md['versions'][0]['version']))
